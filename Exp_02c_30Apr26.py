"""
================================================================================
  ROBOTARIUM SWARM EXPERIMENT v2c: Real-Time Doctor-Patient Medical Response
...
================================================================================
  ...
  WHAT'S NEW IN v2c (vs. Exp_02b_30Apr26.py)
    The v2b upload to the GA Tech server crashed at startup with:
        AttributeError: module 'rps.utilities.barrier_certificates' has no
        attribute 'create_single_integrator_barrier_certificate_with_boundary'.
        Did you mean: 'create_si_barrier_certificate_with_boundary'?
    The Robotarium production server's rps package exports the short-form
    name `create_si_barrier_certificate_with_boundary` while the local rps
    stub bundled with this repo (used by CI smoke tests) exports the
    long-form `create_single_integrator_barrier_certificate_with_boundary`.
    v2c resolves the barrier certificate factory by probing both names so
    the same script runs on both environments unchanged. Behaviour and
    timeline are otherwise identical to v2b.

  MOTIVATION (carried over from v2b)
    ...
    v2b/v2c command at the GRITSBot maximums published by GA Tech:
...
# ─────────────────────────────────────────────────────────────────────────────
# rps API RESOLVER
#   The Robotarium production server and this repo's bundled rps stub use
#   slightly different names for the single-integrator barrier-certificate
#   factory. Probe both so the same upload runs in both environments.
#       server  : create_si_barrier_certificate_with_boundary  (short form)
#       repo    : create_single_integrator_barrier_certificate_with_boundary
# ─────────────────────────────────────────────────────────────────────────────
def _resolve_rps(module, *candidate_names):
    for name in candidate_names:
        fn = getattr(module, name, None)
        if fn is not None:
            return fn
    raise RuntimeError(
        f"None of {candidate_names!r} found in {module.__name__}. "
        f"Installed rps does not match the names this script knows."
    )
...
#     The factory name differs between the server and the repo stub - resolve
#     it dynamically so the same upload works in both places.
# ─────────────────────────────────────────────────────────────────────────────
_make_si_barrier = _resolve_rps(
    bc,
    "create_si_barrier_certificate_with_boundary",
    "create_single_integrator_barrier_certificate_with_boundary",
)
si_barrier_cert = _make_si_barrier(
